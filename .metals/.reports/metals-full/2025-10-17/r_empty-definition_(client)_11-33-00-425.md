error id: file://<WORKSPACE>/client/src/fr/cyu/robafis/Main.scala:map.
  ...
  def update(model: Model): Msg => (Model, Cmd[IO, Msg]) =
    {
      ...
      case Msg.Bluetooth(BluetoothMsg.NotificationReceived(txt)) =>
        ...
        val Point = raw"""\bPOINT\s+(\d+)\b""".r
        val lastPointOpt: Option[Int] =
          completeLines.collect { case Point(n) => n.toInt }.lastOption
        
        val cmd: Cmd[Msg] =
          parseLastPoint(lines)
